...
class Bot():
    ...
    def update_message(self, chat_id, message_id, new_message):
        self.logger.debug(f'Updating message {message_id}: {new_message}')
    ...
    def create_timer(self, timeout_secs, callback, *args, **kwargs):
        if not callable(callback):
            raise TypeError('Expecting a function as input')
        if not timeout_secs:
            raise TypeError("Cannot set a timer for None seconds")
        if args:
